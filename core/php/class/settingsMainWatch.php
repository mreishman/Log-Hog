...
class settingsMainWatch extends core
{
	private $settings;
	...
	private function makeTrueFalseSelect($selectValue)
	{
		return $this->settings->createSelect(
			array(
				0		=> array(
					"value" =>	"true",
					"name"	=>	"True"
	...
	private function generateFileTypeSelect($selectValue)
	{
		return $this->settings->createSelect(
			array(
				0		=> array(
	...
	private function makePatternSelect($selectValue, $selectOptions)
	{
		return $this->settings->createSelect(
			$selectOptions,
			(String)$selectValue,
	...
	public function generateSaveBlock($data = array(), $arrayOfImages)
	{
		$rowNumber = "{{rowNumber}}";
		...
		{
			$saveBlock .=  $this->makePatternSelect($patternSelect, $selectOptions);
		}
		else
		{
		...
		{
			$saveBlock .=  $this->makeTrueFalseSelect($recursiveOptions);
		}
		else
		{
		...
		{
			$saveBlock .= $this->generateFileTypeSelect($FileType);
		}
		else
		{
			$saveBlock .=  $FileType;
		...
		{
			$saveBlock .=   $this->makeTrueFalseSelect($excludeTrimOptions);
		}
		else
		{
		...
		{
			$saveBlock .=   $this->makeTrueFalseSelect($AlertEnabled);
		}
		else
		{
